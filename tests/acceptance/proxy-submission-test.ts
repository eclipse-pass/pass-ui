...
module('Acceptance | proxy submission', function (hooks) {
  ...
  hooks.beforeEach(async function () {
    ...
    const attrs = {
      ...
      lastName: 'Hasgrants',
      locatorIds: [
        `johnshopkins.edu:jhed:${crypto.getRandomValues(new Uint32Array(1))[0]}`,
        `johnshopkins.edu:hopkinsid:${crypto.getRandomValues(new Uint32Array(1))[0]}`,
        `johnshopkins.edu:employeeid:${crypto.getRandomValues(new Uint32Array(1))[0]}`,
        `johnshopkins.edu:jhed:${crypto.getRandomValues(new Uint32Array(1))[0]}`,
      ],
      roles: ['submitter'],
  ...
  async function walkThroughSubmissionFlow(assert: Assert, hasAccount: boolean) {
