...
export default Component.extend({
  ...
  actions: {
    addComment(){
      if (this.message.replace(/(<([^>]+)>)/ig, "").trim().length > 0) {
        let comment = {
          dateTime: getDate(),
          message: this.message.replace(/(<([^>]+)>)/ig, ""),
    ...
    saveComment(orignalComment) {
      let comment = {
        dateTime: getDate(),
        message: orignalComment.message.replace(/(<([^>]+)>)/ig, ""),
        user: this.get('currentUser.user'),
      };
      this.sendAction('saveComment', [this.comments.indexOf(orignalComment), comment]);
    },
